---
title: "Gugus Data Kelompok Pertemuan 1 MPDW USD"
...
data_USD <- read_csv("C:\\Users\\Aquila\\Downloads\\usd_idr_daily_cleaned.csv")
```
```{r}
data_USD
```

Dataset merupakan data time series harian yang mencatat pergerakan nilai tukar USD terhadap IDR. Setiap baris menunjukkan kondisi nilai tukar pada satu hari perdagangan, sedangkan setiap kolom berisi informasi mengenai pergerakan nilai tukar pada hari tersebut.

Kolom Date menunjukkan tanggal pengamatan. 
Kolom Open menunjukkan nilai tukar pada awal perdagangan 
Kolom High menunjukkan nilai tukar tertinggi 
Kolom Low menunjukkan nilai tukar terendah 
Kolom Close menunjukkan nilai tukar pada akhir perdagangan.

# Filter Data into 2024 and Close only
...
```{r}
ts.plot(USD_2024.ts, xlab="Time Period", ylab="Close", 
        main = "USR/IDR Daily Close 2024")
points(USD_2024.ts)
```
...
# bagi data
training_ma <- USD_2024[1:n_train, ]
testing_ma  <- USD_2024[(n_train+1):n, ]
...
```{r}
plot(USD_2024.ts, col="red",main="Plot semua data")
points(USD_2024.ts)
```
```{r}
...
```{r}
ts.plot(USD_2024.ts, xlab="Time Period ", ylab="Close", main= "SMA N=3 Data Close Dollar")
points(USD_2024.ts)
...
error_test.sma = test_ma.ts-data.gab[210:262,3]
...
data.gab2 <- cbind(aktual = c(USD_2024.ts), 
...
```{r}
ts.plot(USD_2024.ts, xlab="Time Period ", ylab="Close", main= "DMA N=3 Data USD/IDR")
points(USD_2024.ts)
...
```{r}
error_test.dma = test_ma.ts-data.gab2[210:262,6]
...
Pada kedua data metode SMA lebih baik dibandingkan dengan metode DMA.
...
# bagi data
training <- USD_2024[1:n_train, ]
testing  <- USD_2024[(n_train+1):n, ]
...
```{r}
#Cara 1 (fungsi ses)
ses.1 <- ses(train.ts, h = 53, alpha = 0.2)
plot(ses.1)
```
```{r}
ses.1
```
```{r}
ses.2<- ses(train.ts, h = 53, alpha = 0.7)
plot(ses.2)
```
```{r}
ses.2
...
Kasus di atas merupakan contoh inisialisasi nilai parameter λ
 dengan nilai alpha 0,2 dan 0,7 dan banyak periode data yang akan diramalkan adalah sebanyak 10 periode.
...
Nilai parameter α
 dari kedua fungsi dapat dioptimalkan menyesuaikan dari error-nya paling minimumnya. Caranya adalah dengan membuat parameter α=
 NULL .
...
akurasi.2
```
Berdasarkan nilai SSE, MSE, RMSE, dan MAPE di antara kedua parameter, nilai parameter λ=0,2
 menghasilkan akurasi yang lebih baik dibanding λ=0,7
